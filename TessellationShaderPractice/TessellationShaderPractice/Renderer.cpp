...
void Renderer::Initialize()
{
	...
	auto terrain = geoGenerator.CreateTerrainPatches(width, height, 20);
	terrain.ConfigureMesh(device, commandList);
	mMeshes.insert({ "terrain", std::move(terrain) });

	stbi_image_free(data);

	// Initialize constant buffer
	mObjectCBs = std::make_unique<UploadBuffer<ObjectConstant>>(device, 5, true);
	...
	mCbvSrvUavDescriptor.CreateDescriptorHeap(device, 5);
	...
	auto terrainTexture = mTextures["terrain"].GetTextureResource();
	mCbvSrvUavDescriptor.CreateShaderResourceView(device, mDirect3D.GetCbvSrvUavDescriptorSize(),
		terrainTexture->GetDesc().Format, D3D12_SRV_DIMENSION_TEXTURE2D, terrainTexture);
	...
	Shader terrainVertexShader;
	Shader terrainHullShader;
	Shader terrainDomainShader;
	Shader terrainPixelShader;
	terrainVertexShader.CompileShader(L"../../Shaders/terrain.hlsl", nullptr, "VSMain", "vs_5_1");
	terrainHullShader.CompileShader(L"../../Shaders/terrain.hlsl", nullptr, "HSMain", "hs_5_1");
	terrainDomainShader.CompileShader(L"../../Shaders/terrain.hlsl", nullptr, "DSMain", "ds_5_1");
	terrainPixelShader.CompileShader(L"../../Shaders/terrain.hlsl", nullptr, "PSMain", "ps_5_1");
	mShaders.insert({ "terrainVS", std::move(terrainVertexShader) });
	mShaders.insert({ "terrainHS", std::move(terrainHullShader) });
	mShaders.insert({ "terrainDS", std::move(terrainDomainShader) });
	mShaders.insert({ "terrainPS", std::move(terrainPixelShader) });
	...
	CreateTessellationPSO(device, "terrain", "default", "terrain");
...
void Renderer::DrawScene()
{
	...
	cbvSrvUavDescriptor.Offset(4, mDirect3D.GetCbvSrvUavDescriptorSize());
	...
	commandList->SetPipelineState(mPSOs["terrain"].Get());
	DrawRenderItems(RenderLayer::Terrain, commandList);
...
void Renderer::CreateRootSignature(ID3D12Device* device, const std::string& rootSignatureName)
{
	...
	slotRootParameters[4].InitAsDescriptorTable(1, &texTable0, D3D12_SHADER_VISIBILITY_ALL);
...
void Renderer::CreateTessellationPSO(ID3D12Device* device, const std::string& psoName, 
	const std::string& rootSignatureName, const std::string& shaderName)
{
	PipelineStateObject pso = nullptr;
	...
	psoDesc.pRootSignature = mRootSignatures[rootSignatureName].Get();
	psoDesc.VS = CD3DX12_SHADER_BYTECODE(mShaders[shaderName + "VS"].GetShader());
	psoDesc.HS = CD3DX12_SHADER_BYTECODE(mShaders[shaderName + "HS"].GetShader());
	psoDesc.DS = CD3DX12_SHADER_BYTECODE(mShaders[shaderName + "DS"].GetShader());
	psoDesc.PS = CD3DX12_SHADER_BYTECODE(mShaders[shaderName + "PS"].GetShader());
	psoDesc.RasterizerState = CD3DX12_RASTERIZER_DESC(D3D12_DEFAULT);
	psoDesc.RTVFormats[0] = mSwapChain.GetBackBufferFormat();
	psoDesc.SampleDesc.Count = m4xMsaaState ? 4 : 1;
...
void Renderer::LoadTextures()
{
	...
	texName = "terrain";
	terrainTexture.CreateTexture(device, commandList, texName.c_str(), L"../../Textures/iceland_heightmap.dds");
	mTextures.insert({ texName, std::move(terrainTexture) });
...
void Renderer::BuildRenderItems()
{
	...
	renderItem.mesh = mMeshes["terrain"];
	world = XMMatrixIdentity();
	XMStoreFloat4x4(&renderItem.world, world);
	...
	renderItem.materialCBIndex = 2;
	renderItem.instanceCount = 1;
	mTerrainRenderItems.push_back(renderItem);

	mAllRenderItems.insert({ RenderLayer::Terrain, mTerrainRenderItems });
